...
const fields = [
  ...
  { name: "qtdAlunos", label: "Qtd. Alunos", type: "number" },
];

const empty = { nome: "", data: "", hora: "", instituicao: "", qtdAlunos: 0 };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20, display: 'flex', gap: 10 }}>
        <input type="text" placeholder="Buscar visita..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: 8, flex: 1, border: '1px solid #ccc', borderRadius: 4 }} />
        <button className="btn btn-secondary" onClick={handleSearch}>Buscar</button>
      </div>
      <div className="table-container">
        <table>
          <thead><tr><th>Nome</th><th>Data</th><th>Instituição</th><th>Alunos</th><th>Ações</th></tr></thead>
          <tbody>
            {items.map((it) => (
              <tr key={it.id}>
                <td>{it.nome}</td><td>{it.data}</td><td>{it.instituicao}</td><td>{it.qtdAlunos}</td>
                <td>
