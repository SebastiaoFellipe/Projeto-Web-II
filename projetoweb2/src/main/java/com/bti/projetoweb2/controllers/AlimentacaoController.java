...
public class AlimentacaoController {
    ...
    @PutMapping("/{id}")
    @Operation(
        summary = "Atualiza uma alimentação existente",
        description = "Atualiza uma alimentação existente com base nos dados fornecidos"
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Alimentação atualizada com sucesso"),
        @ApiResponse(responseCode = "400", description = "Dados inválidos"),
        @ApiResponse(responseCode = "404", description = "Alimentação não encontrada"),
        @ApiResponse(responseCode = "500", description = "Erro interno no servidor")
    })
    public Alimentacao atualizar(@PathVariable Long id, @RequestBody Alimentacao alimentacaoAtualizado) {
        return alimentacaoService.atualizar(id, alimentacaoAtualizado);
    }

    @DeleteMapping("/{id}")
    @Operation(
        summary = "Deleta uma alimentação por ID",
        description = "Deleta uma alimentação específica com base no ID fornecido"
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Alimentação deletada com sucesso"),
        @ApiResponse(responseCode = "404", description = "Alimentação não encontrada"),
        @ApiResponse(responseCode = "500", description = "Erro interno no servidor")
    })
