...
const fields = [
  ...
  { name: "classificacao", label: "Classificação", type: "select", options: [{value:"NAO_AMEACADO", label:"Não Ameaçado"}, {value:"AMEACADO", label:"Ameaçado"}, {value:"EXTINTO", label:"Extinto"}] },
  { name: "dataEntrada", label: "Data Entrada", type: "date" },
  { name: "idade", label: "Idade", type: "number" }
];

const empty = { nome: "", familia: "", especie: "", classificacao: "NAO_AMEACADO", dataEntrada: "", idade: 0 };
  ...
  return (
    <div>
      ...
      <div className="table-container">
        <table>
          <thead><tr><th>Nome</th><th>Família</th><th>Espécie</th><th>Classificação</th><th>Ações</th></tr></thead>
          <tbody>
            {items.map((it) => (
              <tr key={it.id}>
                <td>{it.nome}</td><td>{it.familia}</td><td>{it.especie}</td><td>{it.classificacao}</td>
                <td>
