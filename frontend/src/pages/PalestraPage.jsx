...
const empty = { tema: "", data: "", local: "", palestrante: "" };
...
export default function PalestraPage() {
  ...
  const fetch = async () => {
    try {
      const data = await getItens(API_ENDPOINT, { page, size: 5, tema: searchTerm });
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <button className="btn btn-primary" onClick={() => { setCurrent(empty); setIsEditing(false); setIsModalOpen(true); }}>Nova</button>
      </div>
      <div style={{ marginBottom: 20, display: 'flex', gap: 10 }}>
        <input type="text" placeholder="Buscar por tema..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ padding: 8, flex: 1, border: '1px solid #ccc', borderRadius: 4 }} />
        <button className="btn btn-secondary" onClick={handleSearch}>Buscar</button>
      </div>
      <div className="table-container">
        <table>
          <thead><tr><th>Tema</th><th>Data</th><th>Local</th><th>Palestrante</th><th>Ações</th></tr></thead>
          <tbody>
            {items.map((it) => (
              <tr key={it.id}>
                <td>{it.tema}</td><td>{it.data}</td><td>{it.local}</td><td>{it.palestrante}</td>
                <td>
